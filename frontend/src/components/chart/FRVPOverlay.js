...
function FRVPOverlay({ frvpRange, trades, priceRange, width, height, priceToY, chartArea, }) {
    ...
    return (_jsx("canvas", { ref: canvasRef, className: "frvp-overlay", style: {
            position: 'absolute',
            top: 0,
            left: 0,
            width: `${width}px`,
 ...
 * Volume Profile çiz
 */
function drawVolumeProfile(ctx, profileData, chartArea, priceToY, frvpRange) {
    if (profileData.volumeAtPrice.size === 0)
        return;
 ...
 */
function drawSelectionRange(ctx, frvpRange, chartArea) {
 ...
 */
function drawStatsOverlay(ctx, profileData, chartArea, frvpRange) {
    const padding = 10;
    const lineHeight = 16;
    ...
    y += lineHeight;
    ctx.fillText(`VA High: ${formatPrice(profileData.valueArea.high, 'BTCUSDT')}`, boxX + 8, y);
    y += lineHeight;
    ctx.fillText(`VA Low: ${formatPrice(profileData.valueArea.low, 'BTCUSDT')}`, boxX + 8, y);
}
