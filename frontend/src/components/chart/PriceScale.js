...
function PriceScale({ orderBook, priceRange, height, width = 80 }) {
    ...
    useEffect(() => {
        ...
        drawOrderBookBars(ctx, priceLevels, width);
        // Fiyat etiketlerini çiz
        drawPriceLabels(ctx, priceLevels, width);
    }, [priceLevels, priceRange, height, width]);
    return (_jsx("canvas", { ref: canvasRef, className: "price-scale-canvas", style: {
            width: `${width}px`,
            height: `${height}px`,
            position: 'absolute',
            right: 0,
 ...
 */
function buildPriceLevels(orderBook, priceRange, height) {
    const levels = [];
 ...
 */
function findClosestPriceLevel(levels, targetPrice, maxDistance) {
    if (levels.length === 0)
        return undefined;
    let left = 0;
 ...
 */
function drawOrderBookBars(ctx, levels, width) {
    ...
    levels.forEach((level) => {
        ...
        if (colorInfo.intensity > 0) {
            ...
            // Renk
            const color = getBarColor(colorInfo.color, colorInfo.intensity);
            // Bar çiz (sağdan sola)
            ctx.fillStyle = color;
 ...
 * Fiyat etiketlerini çiz
 */
function drawPriceLabels(ctx, levels, width) {
 ...
 */
function drawSimpleScale(ctx, width, height, priceRange) {
    const steps = 10;
 ...
 */
function getColorIntensity(bidVolume, askVolume) {
 ...
 */
function getBarColor(color, intensity) {
