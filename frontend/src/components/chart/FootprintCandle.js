 ...
 */
export function drawFootprintCandle(ctx, x, kline, footprint, candleWidth, priceToY, options = {
    cellHeight: 8,
    showVolumes: false,
    minCellWidth: 20,
}) {
 ...
 */
function drawTraditionalCandle(ctx, x, kline, candleWidth, priceToY) {
 ...
 */
function drawCandleOutline(ctx, x, kline, candleWidth, priceToY) {
    const yHigh = priceToY(kline.high);
    const yLow = priceToY(kline.low);
    const yOpen = priceToY(kline.open);
    const yClose = priceToY(kline.close);
 ...
 */
function getBuyVolumeColor(buyVolume, totalVolume) {
    if (totalVolume === 0)
        return 'rgba(54, 200, 138, 0.2)';
 ...
 */
function getSellVolumeColor(sellVolume, totalVolume) {
    if (totalVolume === 0)
        return 'rgba(224, 101, 106, 0.2)';
 ...
 */
function drawVolumeNumbers(ctx, x, y, cell, cellWidth, cellHeight) {
 ...
 */
export function getCandleBodyColor(kline) {
