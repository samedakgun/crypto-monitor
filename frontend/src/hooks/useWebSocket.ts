...
import { SubscribeMessage, UnsubscribeMessage, WebSocketMessage } from '@shared/types/websocket';
...
export function useWebSocket({ onMessage, onConnect, onDisconnect }: UseWebSocketOptions) {
  ...
  const subscribe = useCallback((message: SubscribeMessage) => {
  ...
  const unsubscribe = useCallback((message: UnsubscribeMessage) => {
    if (ws.current?.readyState === WebSocket.OPEN) {
