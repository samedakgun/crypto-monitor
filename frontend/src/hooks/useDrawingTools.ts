...
import { useToolStore, Tool } from '../store/toolStore';

// Cizim nesnesi tipleri
export type DrawingType = 'horizontal' | 'vertical' | 'trendline' | 'rectangle' | 'measure';
...
// Varsayilan stiller
const DEFAULT_STYLES: Record<DrawingType, DrawingStyle> = {
...
const toolToDrawingType: Partial<Record<Tool, DrawingType>> = {
...
export function useDrawingTools({
  ...
  const timeToX = useCallback(
    (time: number): number => {
      if (klines.length === 0) return padding.left;
      const effectiveWidth = chartWidth - padding.left - padding.right;
      const step = effectiveWidth / klines.length;

      const index = klines.findIndex((k) => k.openTime >= time);
      if (index === -1) return padding.left + effectiveWidth;
  ...
  const snapToCandle = useCallback(
    (time: number, price: number): number => {
      if (!magnetEnabled || klines.length === 0) return price;
  ...
  const handleMouseMove = useCallback(
    (e: React.MouseEvent<HTMLCanvasElement>) => {
      ...
      const newPoint: DrawingPoint = { x, y: priceToY(price), time, price };
  ...
  const toggleDrawingLock = useCallback((id: string) => {
    setDrawings((prev) =>
  ...
  const renderDrawings = useCallback(
    (ctx: CanvasRenderingContext2D) => {
      ...
      [...drawings, currentDrawing].forEach((drawing) => {
        if (!drawing || !drawing.visible || !drawing.points || drawing.points.length < 1 || !drawing.style) return;
        ...
        switch (type) {
          ...
          case 'measure':
            if (points.length >= 2) {
              ...
              const barCount = klines.filter(
                (k) => k.openTime >= Math.min(p1.time, p2.time) && k.openTime <= Math.max(p1.time, p2.time)
              ).length;
